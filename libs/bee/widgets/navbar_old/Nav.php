...
class Nav extends Widget
{
     ...
     * CONSTANTS
     */
    const ICON_BEFORE_LABEL = 1;
    const ICON_AFTER_LABEL = 0;
    ...
    /**
     * @var boolean this property add css class for <ul> tag for showing menu horizontal.
     * Defaults is false.
     */
    ...
    public function init()
    {
        ...
        if ($this->route === null && Bee::$app->controller !== null) {
            $this->route = Bee::$app->getRequest()->pathInfo;
        }
        if ($this->params === null) {
    ...
    public function renderItem($item)
    {
        ...
        $icon = empty($item['icon']) ? '' : '<span class="' . $this->iconFramework . '"><i class="' . $this->iconPrefix . $item['icon'] . '"></i></span>';
        $label = $encodeLabel ? Html::encode($item['label']) : $item['label'];
        if(!empty($this->options['iconPosition']) && $this->options['iconPosition'] === self::ICON_BEFORE_LABEL) {
            $label = $icon . $label;
        } else {
            $label .= $icon;
        }
        ...
        if (empty($items)) {
            ...
            if ($this->dropDownCaret !== '') {
                $caretLabel = Html::tag('div', $this->dropDownCaret, ['class' => 'caret-conteiner']);
            }
            if(is_array($url)){
                $caretLinkOptions['data-toggle'] = 'dropdown';
                Html::addCssClass($caretLinkOptions, ['widget' => 'dropdown-toggle']);
                Html::addCssClass($caretLinkOptions, 'caret-link');
                $caret = Html::a($caretLabel, '#', $caretLinkOptions);
            } else {
                $linkOptions['data-toggle'] = 'dropdown';
                Html::addCssClass($linkOptions, ['widget' => 'dropdown-toggle']);
                Html::addCssClass($linkOptions, 'caret-icon');
                $caret = '';
        ...
        return Html::tag('li', Html::a($label, $url, $linkOptions) . $caret . $items, $options);
    }
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            $route = $item['url'][0];
            if ($route[0] !== '/' && Bee::$app->controller) {
                $route = Bee::$app->controller->module->getUniqueId() . '/' . $route;
            }
